...
describe("confirmPaymentReceived — permission gate", () => {
  ...
  it("refuses a free-text note that contains a card number (spaced, dashed or plain) and records nothing", async () => {
    seedBookingAndCard();
    currentActor = { id: "admin-1", role: "ADMIN", status: "ACTIVE", paymentPermissions: ["payments.charge"] };
    const { confirmPaymentReceived } = await import("../payment-methods");
    for (const note of ["card 4242 4242 4242 4242 ok", "4242-4242-4242-4242", "paid with 4242424242424242"]) {
      await expect(
        confirmPaymentReceived({ bookingId: "booking-1", paymentMethodId: "pm-1", amount: 100, status: "SUCCEEDED", referenceNote: note })
      ).rejects.toThrow(/never put card numbers in a note/i);
    }
    expect(paymentCharges).toHaveLength(0);
  });

  it("accepts an ordinary reference note (a supplier confirmation code is not a card number)", async () => {
    seedBookingAndCard();
    currentActor = { id: "admin-1", role: "ADMIN", status: "ACTIVE", paymentPermissions: ["payments.charge"] };
    const { confirmPaymentReceived } = await import("../payment-methods");
    const result = await confirmPaymentReceived({ bookingId: "booking-1", paymentMethodId: "pm-1", amount: 100, status: "SUCCEEDED", referenceNote: "Airline ref ABC123, txn 88123" });
    expect(result.status).toBe("SUCCEEDED");
  });
